...
exports.get_account = function (email, cb) {
    ...
    client.query(
        ...
        function (err, results, fields) {
            ...
            cb(results[0] ? results[0].id : null);
        });
};
...
exports.get_key = function (userId, cb) {
    client.query(
        "SELECT wrappedkey, email FROM account WHERE id = ?", [userId],
        function (err, results, fields) {
            if (err) {
                throw err;
            }
            cb(results[0].wrappedkey, results[0].email);
        });
};

var base64urlRegexp = new RegExp(/^[A-Za-z0-9_\-]+$/);
...
exports.set_key = function (userId, wrappedKey, cb) {
    ...
    if (!wrappedKey.match(base64urlRegexp)) {
        cb('{"success": false}');
        return;
    }
